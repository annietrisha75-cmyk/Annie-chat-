...
// Connect to Free MongoDB Cloud Database with patched TLS and IPv4 settings
...
// Start Express IMMEDIATELY so Render passes its port check
...
// Helper: Dynamic Trust Badge Rotator
...
app.post('/register', async (req, res) => {
    try {
        if (!usersCollection) {
            return res.status(503).send("Database still connecting, please try again in a moment.");
        }
...
app.get('/annie-admin-secure-xyz', async (req, res) => {
    try {
        if (!usersCollection) {
            return res.status(503).send("Database loading, please refresh.");
        }
...
// 5. AUTOMATED DAILY EMAIL REMINDER
