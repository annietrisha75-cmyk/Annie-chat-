...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        #setupModal { position: fixed; inset: 0; background: rgba(15,10,14,0.92); display: flex; justify-content: center; align-items: center; z-index: 1000; padding: 20px; }
        .modal-card { background: var(--bg-surface); padding: 25px; border-radius: 16px; border: 1px solid var(--border-color); width: 100%; max-width: 380px; box-shadow: 0 25px 50px rgba(0,0,0,0.8); text-align: center; }
        ...
        .modal-card input { width: 100%; background: var(--bg-base); border: 1px solid var(--border-color); padding: 12px; border-radius: 8px; color: var(--text-main); font-size: 0.95rem; margin-bottom: 12px; outline: none; text-align: center; }
        .modal-card button { width: 100%; background: linear-gradient(135deg, var(--accent-gold), var(--accent-rose)); color: #000; border: none; padding: 12px; border-radius: 8px; font-weight: 700; cursor: pointer; }
...
<body>
    ...
    <script>
        // Automatic Deep-Link Browser Enforcement
        function forceOpenBestBrowser() {
            const url = window.location.href;
            // Forces standard browser intent handling on mobile devices
        ...
        // Auto-login persistence check: If stored, hide modal instantly forever
        if (myEmail) {
        ...
        function lockAndStartSession() {
            ...
            // Permanently lock in local storage
        ...
        async function fetchMessages() {
            ...
            try {
                ...
                // Back up directly to device permanent local storage
        ...
        function renderMessages(messages) {
            ...
            if (container.children.length === messages.length && container.children.length > 0) return;
